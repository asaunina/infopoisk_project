...
# распаковываем zip-файл с моделью
with zipfile.ZipFile('214.zip', 'r') as zip_ref:
    zip_ref.extractall()

keyed_vectors_fasttext = KeyedVectors.load('model.model')


class IndexFasttext:
    '''
    Класс для построения индекса через fasttext.
    '''

    def __init__(self, corpus, keyed_vectors=keyed_vectors_fasttext):
        ...
        # fasttext модель
        self.fasttext_model = keyed_vectors
        # эмбеддинги
        self.doc_embeddings = []
    ...
    def fit(self):
        '''
        Метод для построения эмбеддингов документов.
        '''
        ...
        for lemmas_sentence in self.sentences:
            ...
            for lemma in lemmas_sentence:
                if lemma in self.fasttext_model:
                    vectors_sentence.append(self.fasttext_model[lemma])
            if vectors_sentence:
                sentence_embedding = np.mean(vectors_sentence, axis=0)  
            else:
                sentence_embedding = np.zeros(self.fasttext_model.vector_size)        
        ...
        self.is_fit = True
        # выводим сообщение
        print(f'''Построены эмбеддинги предложений с помощью FastText.''')
        return self
    ...
    def query_embedding(self, query):
        ...
        # получаем усреднённый вектор запроса
        query_vectors = []
        for token in query_tokens:
            if token in self.fasttext_model:
                query_vectors.append(self.fasttext_model[token])
    ...
    def search(self, query, top_n=5):
        ...
        if not self.is_fit:
            raise ValueError("Эмбеддинги не построены. Необходимо сначала вызвать метод .fit()")
